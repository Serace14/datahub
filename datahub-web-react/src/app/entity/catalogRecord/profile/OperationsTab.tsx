...
import { GetCatalogRecordRunsQuery, useGetCatalogRecordRunsQuery } from '@graphql/catalogRecord.generated';
...
export const OperationsTab = () => {
    ...
    const datas: GetCatalogRecordRunsQuery[] = [];
    allUrns.forEach((entityUrn) => {
        ...
        const { loading, data } = useGetCatalogRecordRunsQuery({
            variables: {
                urn: entityUrn,
    ...
    let dataRuns: NonNullable<GetCatalogRecordRunsQuery['catalogRecord']>['runs'] | undefined;
    if (datas.length > 0) {
        let numWithRuns = 0;
        for (let i = 0; i < datas.length; i++) {
            if (datas[i]?.catalogRecord?.runs?.total) {
                numWithRuns++;
            }

            if (dataRuns && dataRuns.runs) {
                dataRuns.runs.push(...(datas[i]?.catalogRecord?.runs?.runs || []));
                dataRuns.total = (dataRuns.total ?? 0) + (datas[i]?.catalogRecord?.runs?.total ?? 0);
            } else {
                dataRuns = JSON.parse(JSON.stringify(datas[i]?.catalogRecord?.runs));
            }
        }
    ...
    const containsJobs = tableData?.some((run) => run.parentTemplate?.type !== EntityType.CatalogRecord);
