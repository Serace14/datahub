...
from datahub.emitter.mce_builder import make_dataset_urn, make_domain_urn, make_tag_urn, make_term_urn
...
from distribution_functions import create_distribution_and_attach
    ...
    for xp in xpath_list:
        ...
        if elements:
            ...
            if hasattr(el, "text") and el.text:
                ...
                return el.strip()
            elif hasattr(el, "get") and el.get("codeListValue"):
                return el.get("codeListValue")
    return None


# ============================================================
#                   FUNCIÓN PRINCIPAL (DATASET)
# ============================================================
def create_dataset_from_catalog_record(catalog_record_urn: str) -> str:
    """
    Crea un Dataset a partir del CatalogRecord y devuelve el dataset_urn.
    NO hace enriquecimiento: solo creación + asociación.
    """

    # 1. Obtener PlatformResource asociado
    query = """
    ...
    relationships = res["catalogRecord"]["relationships"]["relationships"]

    if not relationships:
        raise ValueError(f"No PlatformResource asociado a {catalog_record_urn}")
    ...
    # 3. Extraer datos
    title = extract_text(tree, ns, ["//gmd:title/gco:CharacterString", "//dc:title"]) or \
            f"dataset-{random.randint(1000,9999)}"

    description = extract_text(tree, ns, ["//gmd:abstract/gco:CharacterString", "//dc:description"]) \
                  or "Dataset derivado de CatalogRecord"

    primary_key = getattr(aspect_info, "primaryKey", None)

    identifier = extract_text(tree, ns, ["//dc:identifier", "//gmd:fileIdentifier/gco:CharacterString"])

    dataset_id = (primary_key or identifier or title).replace(" ", "_").lower()
    dataset_urn = make_dataset_urn("geoserver", dataset_id, "PROD")

    # 4. Emit dataset básico
    dataset_props = DatasetPropertiesClass(name=title, description=description)
    emitter.emit(MetadataChangeProposalWrapper(entityUrn=dataset_urn, aspect=dataset_props))

    # 5. Asociar Dataset → CatalogRecord
    patch = DatasetPatchBuilder(dataset_urn)
    patch.set_structured_property("urn:li:structuredProperty:catalog", catalog_record_urn)
...
# ============================================================
def enrich_dataset(dataset_urn: str, catalog_record_urn: str):
    """
    Enriquecimiento completo del Dataset:
    - ID
    - Bounding Box
    - DataProduct
    - Fecha
    ...
    # Obtener PlatformResource desde el CatalogRecord
    query = """
    query getCatalogWithRelationships($urn: String!) {
      catalogRecord(urn: $urn) {
        relationships(input: {types: ["AssociatedWith"], direction: OUTGOING}) {
    ...
    if id_value:
        patch = DatasetPatchBuilder(dataset_urn)
        patch.set_structured_property("urn:li:structuredProperty:id", id_value)
        for mcp in patch.build():
            emitter.emit(mcp)
    ...
    dp_properties = DataProductPropertiesClass(
        ...
        description=f"DataProduct generado automáticamente a partir del dataset {id_value}",
    ...
    emitter.emit(MetadataChangeProposalWrapper(entityUrn=domain_urn, aspect=DomainPropertiesClass(name=domain_name,
    ...
    # ========================
    orgs = [el.text.strip() for el in
            tree.findall(".//gmd:pointOfContact//gmd:organisationName//gco:CharacterString", ns) if el.text]
    if orgs:
        org_name = orgs[0]
    ...
    create_distribution_and_attach(emitter, dataset_urn, tree)

    print(f"🔥 Dataset {dataset_urn} enriquecido correctamente.")
